...
<body>
    ...
    $saveout = $_POST['saveout'];
    $pidd = trim($_POST['pidd']);
    ...
    $bod = trim($_POST['bod']);
    $sddate = trim($_POST['startdate']);
    $eddate = trim($_POST['enddate']);
    ...
    $carregistration = trim($_POST['carregistration']);
    if (empty($_POST['parkingpst'])) {
        $parkingpst = '';
    } else {
        $parkingpst = trim($_POST['parkingpst']);
    }
    ...
    $fleetcardcarbrand = trim($_POST['fleetcardcarbrand']);
    $fleetcardcarregistration = trim($_POST['fleetcardcarregistration']);
    $fleetcardfuel = trim($_POST['fleetcardfuel']);
    $fleetcardlimit = trim($_POST['fleetcardlimit']);
    $fleetcarddocuments = trim($_POST['fleetcarddocuments']);
    ...
    // if (!empty($fleetcardcarbrand) && !empty($fleetcardcarregistration) && !empty($fleetcardfuel) && !empty($fleetcardlimit) && !empty($fleetcarddocuments)) {
    //     $card = 'card';
    // }
    $mobilephonenumber = trim($_POST['mobilephonenumber']);
    $officeequipment = trim($_POST['officeequipment']);
    $other1 = trim($_POST['other1']);
    $other2 = trim($_POST['other2']);
    $other3 = trim($_POST['other3']);
    $other4 = trim($_POST['other4']);
    $other5 = trim($_POST['other5']);
    $other6 = trim($_POST['other6']);
    ...
    // $iduser = $nameen . '.' . strtolower(substr($lastnameen, 0, 1));

    date_default_timezone_set('Asia/Bangkok');
    $date_now = date('d/m/Y H:i');
    $ad = $_SESSION["UserID"];
    $log = $ad . " " . $date_now . " " . " edit user " . $nameen;
    $admin =  $date_now . " " . " edit user " . $nameen;
    ...
    if (empty($_FILES["img"]["tmp_name"])) {
        ...
        mysqli_query($conn, "UPDATE `employee` SET `pid`='$pid',`iduser`='$iduser',`nametitles(TH)`='$n',`nameth`='$nameth',`lastnameth`='$lastnameth', `nickname`='$nickname',`nametitles(EN)`='$nn',`nameen`='$nameen',`lastnameen`='$lastnameen',`position`='$position',`department`='$department',`phone`='$phone',`telephone`='$telephone',`email`='$email',`bod`='$bod',`startdate`='$sddate',`enddate`='$eddate',`img`='$filedata',`status`='$check',`description`='$des',`hardware`='$hardware',`software`='$software',`application`='$application',`network`='$network',`other`='$other',`carbrand`='$carbrand',`carregistration`='$carregistration',`parkingpst`='$parkingpst',`parking`='$parking', `card`='$card' ,`fleetcardcarbrand`='$fleetcardcarbrand',`fleetcardcarregistration`='$fleetcardcarregistration',`fleetcardfuel`='$fleetcardfuel',`fleetcardlimit`='$fleetcardlimit',`fleetcarddocuments`='$fleetcarddocuments',`mobilephonenumber`='$mobilephonenumber',`officeequipment`='$officeequipment',`other1`='$other1',`other2`='$other2',`other3`='$other3',`other4`='$other4',`other5`='$other5',`other6`='$other6',`log`='$log' WHERE `pid` = '$pidd'");
        echo "<script>";
        echo "Swal.fire('แก้ไขข้อมูลเรียบร้อยแล้ว','','success').then(function() {window.location.href='profile?Page=$page'})";
        echo "</script>";
    } else {
    ...
    // mysqli_query($conn, "UPDATE `employee` SET `pid`='$pid',`iduser`='$iduser',`nameth`='$nameth',`lastnameth`='$lastnameth',`nameen`='$nameen',`lastnameen`='$lastnameen',`idcard`='$idcard',`position`='$position',`department`='$department',`phone`='$phone',`telephone`='$telephone',`email`='$email',`startdate`='$sdy',`enddate`='$edy' WHERE `pid` = '$pidd'");
    // mysqli_query($conn, "UPDATE `employee` SET `pid`='$pid',`iduser`='$iduser',`nameth`='$nameth',`lastnameth`='$lastnameth',`nameen`='$nameen',`lastnameen`='$lastnameen',`position`='$position',`department`='$department',`phone`='$phone',`telephone`='$telephone',`email`='$email',`startdate`='$sddate',`enddate`='$eddate',  `log`='$log', `img`='$filedata',`status` = $check  WHERE `pid` = '$pidd'");
    mysqli_query($conn, "UPDATE `user` SET `log`='$admin' WHERE `user` = '$ad'");
    // header('Location: profile?Page=' . $page);
    // echo "<script>";
    // echo "Swal.fire('เพิ่มข้อมูลเรียบร้อยแล้ว','','success').then(function() {window.location.href='profile?Page=$page'})";
    // echo "</script>";
    use PHPMailer\PHPMailer\PHPMailer;
    use PHPMailer\PHPMailer\Exception;

    if (!empty($_POST['sandemail'])) {
        error_reporting(0);
        ...
        if (empty($other)) {
            $oth = '<img width="15" height="15" src="icon\uncheck.gif">';
        } else {
            $oth = '<img width="15" height="15" src="icon\check.gif">';
        }
        ...
        if (isset($network)) {
            $network = '<img src="icon/check.gif" width="15" height="15">';
        } else {
            $network = '<img src="icon/uncheck.gif" width="15" height="15">';
        }
                    ...
                    <table class='a' style='margin-left: auto;margin-right: auto;'>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style='font-size: 15px;'>
                                $spacecom2 $fleetcardcarbrand
                                </td>
                                <td style='font-size: 15px;'>
                                $spacecom2 ทะเบียนรถ
                                </td>
                                <td style='font-size: 15px;'>
                                $spacecom2 $fleetcardcarregistration
                                </td>
                                </tr>
                                </table>
                                </td>
                            ...
                            <tr>
                                ...
                                <td style='font-size: 15px;'>
                                $spacecom2 $fleetcardfuel
                                </td>
                                <td style='font-size: 15px;'>
                                $spacecom2 วงเงิน
                                </td>
                                <td style='font-size: 15px;'>
                                $spacecom2 $fleetcardlimit
                                </td>
                                </tr>
                                </table>
                                </td>
                            ...
                            <tr>
                                ...
                                <td style='font-size: 15px;width: 10px;'>$other1</td>
                                </tr>
                                </table>
                                </td>
                                <td>
                                ...
                                <td style='font-size: 15px;'>$other4</td>
                                </tr>
                                </table>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td style='font-size: 15px;width: 10px;'>$other2</td>
                                </tr>
                                </table>
                                </td>
                                <td>
                                ...
                                <td style='font-size: 15px;'>$other5</td>
                                </tr>
                                </table>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td style='font-size: 15px;width: 10px;'>$other3</td>
                                </tr>
                                </table>
                                </td>
                                <td>
                                ...
                                <td style='font-size: 15px;'>$other6</td>
                                </tr>
                                </table>
                                </td>
                            </tr>
    ...
    ?>
        ...
        $body = "<table style='width: 100%;border-collapse: collapse;border: 1px solid black;text-align: center;'>
                    ...
                    <tr>
                        ...
                        <td style='border: 1px solid black;'>$sddate</td>
                        <td style='border: 1px solid black;'>$eddate</td>
        ...
        try {
            ...
            $text = "ขอแจ้งแก้ไขข้อมูลของพนักงาน";
    ...
    if (!empty($eddate) && isset($saveout)) {
        ...
        try {
            ...
            //$mail->addAddress('waranon@pst.co.th');
            // $mail->addAddress('receiver2@gfg.com', 'Name');
            $text = "ขอแจ้งข้อมูลการลาออกของพนักงาน";
